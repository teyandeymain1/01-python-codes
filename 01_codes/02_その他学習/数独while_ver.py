...
def solve_Sudoku_main(varList, inputMatrix):
    ...
    savePoint = []
    loc = 0
    while True: #locが"-1"(空欄が一つもない)になるまで、実行。
...
def main():
    ...
    savedMatrixList = []
    savePoint = []
    varList1.extend([set(range(1, varList1[0]+1))]) #各マスに入るすべての数字の候補(9×9の数独なら1~9まで)
    #関数の実行
    ansMatrix = solve_Sudoku_main(varList1, matrix1)
    print_matrix(ansMatrix)
    print(is_Sudoku_correct(varList1, ansMatrix))
    #----------時間測定終了-----------
    end = time.time()
