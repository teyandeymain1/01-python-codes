...
def make_intList():
    ...
    return intList
#----------numpyによる行列の作成-------------------
def make_matrix_by_numpy(varList):
    ...
    for i in range(row):
        matrix[i] = make_intList() #行ごとに好きな文字を行列に代入できる
...
def find_empty(inputMatrix):
    #配列の中にタプルを代入
    emptyLocList = np.array([(r, c) for r, rowList in enumerate(inputMatrix) for c, item in enumerate(rowList) if item == 0]) #"for r, rowList"->"for c, item"の順で動く 
...
def make_rowSet(varList, inputMatrix):
    ...
    rowSet = np.unique(inputMatrix[varList])

    return rowSet
#-------------------------内包表記による集合(列)の作成-------------------------------------
def make_colSet(varList, inputMatrix):

    columnSet = np.unique(inputMatrix[:, varList]) #inputMatrix[行の範囲,　列の範囲]
...
def make_SqSet(varList, inputMatrix):
    ...
    threeSqList = []
    for i in range(3):
        threeSqList = np.append(threeSqList, inputMatrix[row+i][col:(col+3)])

    threeSqSet = np.unique(threeSqList)
...
def make_answerSet(varList, varSet, inputMatrix):
    ...
    #集合作成
    existSet = np.union1d(make_rowSet(row, inputMatrix), make_colSet(col, inputMatrix), make_SqSet(varList, inputMatrix)) #行、列、3×3の集合を合体
    answerSet = np.setdiff1d(varSet, existSet) #引数として与えられた1~9までの数を要素とする集合からexistSetを引く。    
    return answerSet
...
def main():
    ...
    matrix1 = make_matrix_by_numpy(varList1)
    #-----時間測定開始------
    start = time.time()
    #変数定義
    savedMatrixList = []
    savePoint = []
    varList1.extend([np.unique(range(1, varList1[0]+1)), savedMatrixList, savePoint]) #各マスに入るすべての数字の候補(9×9の数独なら1~9まで)
    #関数の実行
