...
def solve_Sudoku_main(varList, inputMatrix):
    ...
    if loc == -1: #与えられた数独に空欄がないとき
        print_matrix(inputMatrix) #locが"-1"(空欄が一つもない)とき、数独を表示する関数を実行。
        return print(is_Sudoku_correct(varList, inputMatrix))
        ...
        return solve_Sudoku_main(varList, inputMatrix)
