...
def solve_answer(inMatrix, y):
    return list(math.sqrt(make_distance(inMatrix[i], inMatrix[i+1])) for i in range(y))
#=====================================================
readline = sys.stdin.readline
y, = map(int, readline().replace('\n','').replace('\r','').split())
#引数が一つの時はここに , を入れること                          #空欄がないときは .split()を消す
inMatrix1 = make_matrix_without_numpy(y)
inMatrix1 = [[0, 0]] +  inMatrix1 + [[0, 0]]

#
print(sum(solve_answer(inMatrix1, (y+1))))